...
import styles from './DeleteMovementDialog.styles';
...
export default function DeleteMovementDialog({ visible, accountId, item, onClose, onDelete }: Props) {
 const [deleting, setDeleting] = useState(false);
 const isFishertonSpecial = useMemo(
  () => shouldApplyFishertonMirrorRule(api.defaults.baseURL, accountId),
  [accountId],
 );
 const affectsClient =
  ...
  <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
   <Pressable style={styles.backdrop} onPress={onClose}>
    <Pressable style={styles.card} onPress={() => {}}>
     ...
     {affectsClient ? (
      <Text style={styles.affectText}>Esta operación afecta al cliente en una operacion de compra o venta.</Text>
     ) : null}
